...
async function onMessage({ from, text, raw, sock }) {
  try {
    ...
    // handle quick paid message -> mark order if exists
    if (/^paid$|^done$|ho gaya|complete/i.test(text)) {
      // mark latest pending order for this phone (admin will verify)
      ...
      // reply
      try { await sock.sendMessage(from, { text: 'Shukriya — admin ko notification bhej di gayi hai. Hum verify kar ke aapko bata denge.' }); } catch(e){}
      return;
    }
...
(async () => {
  ...
  } catch (err) { pino.error({ err }, 'startup failed'); process.exit(1); }
})();
